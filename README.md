...
## Estrutura (Inspirada no Nuxt / Monólito Modular)

Nenhuma pasta de código (`components`, `lib`, `hooks`, `utils`) fica solta na raiz. Tudo segue uma convenção simétrica e previsível:
...
shared/                Recursos compartilhados entre app e todas as layers
  components/          Design system global e UI compartilhada (ex: ui/)
  lib/                 SDK do Payload e utilitários globais (ex: sdk.ts, utils.ts)
  ...
  pwa/                 Hooks Workbox, registro e componentes PWA
  sync/                Engine de sincronização offline-first

app/                   Aplicação principal (Default Layer)
  components/          Componentes exclusivos do app (app-shell, bottom-tabs, etc.)
  lib/                 Libs exclusivas do app (query-client.ts)
  hooks/               Hooks exclusivos do app
  utils/               Utilitários exclusivos do app
  routes/              TanStack Router file-based (páginas React)
  main.tsx             RouterProvider + QueryClient
  routeTree.gen.ts     GERADO — nunca edite (rode `bun run routes:gen`)
  sw.ts                Service Worker Workbox

layers/<nome>/         Camadas e sub-aplicações isoladas (ex.: admin, auth, kiosk)
  ...
  routes/              TanStack Router file-based da camada
  main.tsx             RouterProvider com basepath: '/<nome>'
  index.html           Entrypoint HTML da camada
```
