...
- **Arquitetura em Camadas (Inspirada no Nuxt / Monólito)**:
  - **Proibido colocar pastas soltas de código na raiz** (como `components/`, `lib/`, `hooks/`, `utils/`).
  - **`shared/`** — Tudo que é compartilhado entre `app/` e `layers/` reside aqui: `shared/(components, lib, hooks, utils, stores, pwa, sync)`.
  - **`app/`** — Aplicação principal / camada padrão. Recursos exclusivos do app ficam encapsulados aqui: `app/(components, lib, hooks, routes, utils, main.tsx, index.html, sw.ts)`.
  - **`layers/<nome>/`** — Camadas e sub-aplicações isoladas (ex: `admin`, `auth`, `kiosk`). Seguem rigorosamente a mesma estrutura: `layers/<nome>/(components, lib, hooks, routes, utils, main.tsx, index.html)`.
